...
void do_bgfg(char **argv) {
  ...
  if (!strcmp("bg", argv[0])) {
    ...
    app_error("do_bgfg: error argv[0]");
  }
  int jid = 0;
  pid_t pid = 0;
  if (argv[1] == NULL) {
...
void waitfg(pid_t pid) {
  ...
  while (1) {
    ...
    if (fgpid(jobs) == 0 || fgpid(jobs) != pid) {
      wait_done = 1;
    }
    if (sigprocmask(SIG_SETMASK, &prev_mask, NULL)) {
      unix_error("sigprocmask");
...
void sigchld_handler(int sig) {
  ...
  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) != 0) {
    ...
    //printf("pid: %d, exited(%d), signaled(%d), stopped(%d), continued(%d)\n",
    //       pid, WIFEXITED(status), WIFSIGNALED(status), WIFSTOPPED(status), WIFCONTINUED(status));
...
void listjobs(struct job_t *jobs) {
  ...
  for (i = 0; i < MAXJOBS; i++) {
    if (jobs[i].pid != 0) {
      ...
      switch (jobs[i].state) {
        case BG: printf("Running ");
          break;
        case FG: printf("Foreground ");
          break;
        case ST: printf("Stopped ");
          break;
        default:
