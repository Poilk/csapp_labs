...
word f_pc = [
	...
	M_icode == IJXX && !M_Cnd && M_valE == 0: M_valA;
...
word f_predPC = [
	f_icode == IJXX && f_ifun != UNCOND && D_icode != IOPQ && E_icode == IOPQ && e_Cnd : f_valC;
	f_icode == IJXX && f_ifun != UNCOND && D_icode != IOPQ && E_icode == IOPQ && !e_Cnd : f_valP;
...
word aluA = [
	...
	E_icode == IJXX && M_icode != IOPQ && W_icode == IOPQ : 1;
	E_icode == IJXX : 0;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && !(M_icode != IOPQ && W_icode == IOPQ)) ||
	# Stalling at fetch while ret passes through pipeline
